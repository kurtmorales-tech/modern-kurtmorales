...
const allowedOrigins = (
	process.env.CORS_ORIGINS || "http://localhost:3000,https://kurtmorales.com"
)
	.split(",")
	...
	async fetch(request) {
		...
		try {
			if (pathname === "/") {
				return text(
					request,
					`KurtMorales Bun backend running on :${port}\nDB: ${getDbPath()}`,
				);
			}
			...
			if (pathname === "/api/posts" && request.method === "GET") {
				const docs = listPosts({
					status:
						(searchParams.get("where[status][equals]") as
							| "draft"
							| "published"
							| null) ?? undefined,
			...
			if (pathname === "/api/subscribers" && request.method === "GET") {
				const docs = listSubscribers(
					parseLimit(searchParams.get("limit"), 1000),
				);
				return json(request, {
					docs,
					totalDocs: docs.length,
			...
			if (newsletterMatch) {
				...
				if (request.method === "PATCH") {
					...
					const doc = updateNewsletter(id, {
						...
						preheader:
							typeof body.preheader === "string" ? body.preheader : undefined,
						contentMarkdown:
						...
						status:
							typeof status === "string"
								? (status as "draft" | "sending" | "sent")
								: undefined,
						sentAt: typeof body.sentAt === "string" ? body.sentAt : undefined,
						recipientsCount:
							typeof body.recipientsCount === "number"
...
console.log(
	`🚀 KurtMorales backend listening on http://localhost:${server.port}`,
);
