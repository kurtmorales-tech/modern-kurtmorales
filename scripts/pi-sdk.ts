...
const BACKEND_URL =
  process.env.PUBLIC_BACKEND_URL || process.env.BACKEND_URL || 'http://localhost:3001';
    ...
    async execute(_toolCallId, params) {
      ...
      try {
        ...
        const lines = data.docs.map(
          (post: any) => `[${post.status}] ${post.title} — ${post.slug} (${post.date})`,
        );
        return {
          content: [
            {
    ...
    handler: async (_args, ctx) => {
      ...
      try {
        ...
        const projects = projectsRes.ok
          ? ((await projectsRes.json()) as PagedResult<unknown>).totalDocs
          : 'err';
        ctx.ui.notify(`Backend: ${posts} posts, ${projects} projects — ${BACKEND_URL}`, 'info');
      } catch (e: any) {
