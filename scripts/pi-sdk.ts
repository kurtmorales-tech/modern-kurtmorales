...
import { personalAnalyticsExtension } from "./personal-analytics-extension";
...
type PagedResult<T> = { docs: T[]; totalDocs: number };
...
function contentExtension(pi: ExtensionAPI) {
	pi.registerTool({
		...
		description:
			"Check if the Bun backend is running and report post/project counts",
		...
		async execute() {
			try {
				...
				const posts = postsRes.ok
					? ((await postsRes.json()) as PagedResult<unknown>).totalDocs
					: "err";
				const projects = projectsRes.ok
					? ((await projectsRes.json()) as PagedResult<unknown>).totalDocs
					: "err";
				return {
					content: [
						{
							type: "text",
							text: `✅ Backend online — ${posts} posts — ${projects} projects — API: ${BACKEND_URL}`,
						},
					],
					details: {},
				};
				...
				return {
					content: [
						{ type: "text", text: `❌ Backend unreachable: ${e.message}` },
					],
					details: {},
				};
			}
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			status: Type.Optional(
				Type.String({ description: "Filter: published | draft" }),
			),
		}),
		async execute(_toolCallId, params) {
			...
			try {
				...
				const data = (await res.json()) as PagedResult<any>;
				const lines = data.docs.map(
					(post: any) =>
						`[${post.status}] ${post.title} — ${post.slug} (${post.date})`,
				);
				return {
					content: [
						{
							type: "text",
							text: `${data.totalDocs} posts:\n${lines.join("\n")}`,
						},
					],
					details: {},
				};
		...
		async execute() {
			try {
				const res = await fetch(
					`${BACKEND_URL}/api/projects?sort=order&limit=50`,
				);
				if (!res.ok) throw new Error(`HTTP ${res.status}`);
				const data = (await res.json()) as PagedResult<any>;
				const lines = data.docs.map(
					(project: any) =>
						`[${project.type}] ${project.title} — ${project.tech} → ${project.link}`,
				);
				return {
					content: [
						{
							type: "text",
							text: `${data.totalDocs} projects:\n${lines.join("\n")}`,
						},
					],
					details: {},
				};
		...
		handler: async (_args, ctx) => {
			...
			try {
				...
				const projects = projectsRes.ok
					...
					: "err";
				ctx.ui.notify(
					`Backend: ${posts} posts, ${projects} projects — ${BACKEND_URL}`,
					"info",
				);
			} catch (e: any) {
...
async function main() {
	...
	const settingsManager = SettingsManager.create(PROJECT_ROOT, getAgentDir());
	...
	const resourceLoader = new DefaultResourceLoader({
		...
		settingsManager,
		extensionFactories: [contentExtension, personalAnalyticsExtension],
		systemPromptOverride:
			() => `You are a project assistant for the KurtMorales portfolio.

## Architecture
Monorepo: apps/web/ (React + Vite) + backend/ (Bun + SQLite)
...
- /analytics: Open personal analytics dashboard
