...
async function main() {
  ...
  const candidates = dedupeByLink(await collectFeedItems(DEFAULT_RSS_SOURCES)).filter((item) => !seenLinks.has(normalizeLink(item.link)))
  ...
  const webPosts: WebPost[] = nextPosts.map(({ generatedAt, sourceName, sourceFeedUrl, sourceTitle, sourceLink, ...post }) => post)
...
async function readStore(): Promise<CmsGeneratedStore> {
  try {
    ...
    return { updatedAt: parsed.updatedAt ?? new Date(0).toISOString(), posts: Array.isArray(parsed.posts) ? parsed.posts : [] }
  } catch {
...
async function collectFeedItems(sources: RssSource[]): Promise<FeedItem[]> {
  ...
  return results.flatMap((result) => (result.status === 'fulfilled' ? result.value : [])).filter(isRelevantFeedItem)
}

async function fetchFeedItems(feed: RssSource): Promise<FeedItem[]> {
  try {
    const response = await fetch(feed.url, {
      headers: { accept: 'application/rss+xml, application/atom+xml, text/xml, */*', 'user-agent': 'KurtMoralesRSSCron/1.0' },
